...
impl MacroExpander {
    ...
    pub fn expand(&mut self, ast: AstNode) -> AstNode {
        match ast {
            ...
            AstNode::MacroCall { name, args, span } => {
                let expanded_args: Vec<AstNode> = args.into_iter().map(|a| self.expand(a)).collect();
                ...
                AstNode::MacroCall { name, args: expanded_args, span }
            }
            AstNode::FunctionDef { name, params, return_type, body, decorators, span } => {
                AstNode::FunctionDef {
            ...
            AstNode::LetStmt { name, mutable, ty, expr, span } => {
                AstNode::LetStmt {
                    name, mutable, ty,
            ...
            AstNode::IfExpr { condition, then_body, else_body, span } => {
                AstNode::IfExpr {
            ...
            AstNode::ForLoop { var, iterable, body, span } => {
                AstNode::ForLoop {
                    var,
            ...
            AstNode::FunctionCall { func, args, span } => {
            ...
            AstNode::MatchExpr { scrutinee, arms, span } => {
                AstNode::MatchExpr {
            ...
            AstNode::StructLiteral { name, fields, span } => {
                AstNode::StructLiteral {
                    name,
                    fields: fields.into_iter().map(|(n, e)| (n, self.expand(e))).collect(),
                    span,
                }
            }
            AstNode::MemberAccess { object, member, span } => {
    ...
    fn match_pattern_top(&self, pattern: &Pattern, args: &[AstNode], bindings: &mut HashMap<String, AstNode>) -> bool {
        match pattern {
            Pattern::Tuple(patterns) => {
                if patterns.len() == args.len() {
                    for (p, a) in patterns.iter().zip(args.iter()) {
                        if !self.match_single_pattern(p, a, bindings) { return false; }
                    }
                    true
                } else {
                    false
    ...
    fn match_single_pattern(&self, pattern: &Pattern, arg: &AstNode, bindings: &mut HashMap<String, AstNode>) -> bool {
        match (pattern, arg) {
    ...
    fn substitute(&self, template: AstNode, bindings: &HashMap<String, AstNode>) -> AstNode {
        match template {
            ...
            AstNode::Block(nodes) => {
                AstNode::Block(nodes.into_iter().map(|n| self.substitute(n, bindings)).collect())
            }
            AstNode::FunctionDef { name, params, return_type, body, decorators, span } => {
                AstNode::FunctionDef {
                    name, params, return_type,
            ...
            AstNode::StructLiteral { name, fields, span } => {
                AstNode::StructLiteral {
                    name,
                    fields: fields.into_iter().map(|(n, e)| (n, self.substitute(e, bindings))).collect(),
                    span,
                }
            }
            AstNode::MemberAccess { object, member, span } => {
