use cranelift_codegen::ir::{AbiParam, types, Value as IrValue, InstBuilder};
...
impl Compiler {
    ...
    pub fn compile_program(&mut self, ast: &AstNode) -> Result<*const u8, String> {
        if let AstNode::Program(nodes) = ast {
            ...
            let main_id = self.module.get_name("main").ok_or("No 'main' function found")?;
    ...
    fn compile_function(&mut self, name: &str, params: &Vec<crate::ast::Param>, body: &AstNode) -> Result<(), String> {
        let func_id = if let Some(cranelift_module::FuncOrDataId::Func(id)) = self.module.get_name(name) {
            id
        } else {
    ...
    fn translate_node(&mut self, node: &AstNode) -> Result<IrValue, String> {
        match node {
            ...
            AstNode::Identifier(name, _) => {
                let var = self.variables.get(name).ok_or_else(|| format!("Unknown variable: {}", name))?;
            ...
            AstNode::FunctionCall { func, args, .. } => {
                if let AstNode::Identifier(name, _) = &**func {
                    ...
                    if let cranelift_module::FuncOrDataId::Func(id) = func_id {
                        let local_func = self.module.declare_func_in_func(id, &mut self.builder.func);
            ...
            _ => Err(format!("Cranelift translation not implemented for node: {:?}", node)),
        }
    }
}
