...
use crate::ir::{IRBlock, IRFunction, Opcode};
...
impl BytecodeOptimizer {
    ...
    fn optimize_block(
        ...
        for (idx, instr) in block.instructions.iter().enumerate() {
            ...
            instructions.push(SpecializedInstruction {
                opcode: specialized,
            });
        }
    ...
    fn specialize_instruction(
        ...
        match opcode {
            ...
            Opcode::PushInt(v) if *v >= -128 && *v <= 127 => SpecializedOpcode::PushSmallInt(*v),
...
impl OptimizedVM {
    ...
    pub fn load_program(&mut self, ir_functions: &[IRFunction]) {
        for func in ir_functions {
            ...
            self.functions
                .insert(func.name.clone(), Arc::new(specialized));
        }
    }
    ...
    pub fn execute(&mut self, func_name: &str, args: Vec<Value>) -> Result<Value, String> {
        ...
        let jit_info = self
            .functions
            .get(func_name)
            .and_then(|f| f.jit_code.map(|ptr| (ptr, f.clone())));
    ...
    fn execute_jit(
        ...
        let res = match func.params.len() {
            ...
            _ => {
                return Err(format!(
                    "JIT execution only supports up to 2 arguments in this POC, got {}",
                    func.params.len()
                ))
            }
        };
        ...
        loop {
            ...
            match &instr.opcode {
                ...
                SpecializedOpcode::CallFast(callee_name, arg_count, site_id) => {
                    ...
                    let saved_locals =
                        std::mem::replace(&mut self.context.locals, FastLocals::new(args.len()));
                    for (idx, arg) in args.into_iter().enumerate() {
                        self.context.locals.set(idx, arg);
                    }
                    ...
                    let target = if let Some(cached) =
                        self.context.inline_cache.get_cached_function(*site_id)
                    {
                        cached.clone()
                    } else {
                        let f = self
                        ...
                        self.context
                            .inline_cache
                            .cache_function(*site_id, f.clone());
                        f
                    };
                    ...
                    match opcode {
                        ...
                        crate::ir::Opcode::LoadVar(name) => {
                            ...
                            if let Some(&slot) = func.param_slots.get(name) {
                                ...
                                self.context.inline_cache.get_cached_slot(name)
                            {
                                let value = self.context.locals.get(slot).clone();
                                self.context.push(value);
                            } else {
    ...
    fn test_bytecode_specialization() {
        ...
        let ir_func = IRFunction {
            ...
            blocks: vec![IRBlock {
                ...
                instructions: vec![
                    IRInstruction {
                        opcode: Opcode::LoadVar("a".to_string()),
                    },
                    IRInstruction {
                        opcode: Opcode::LoadVar("b".to_string()),
                    },
                    IRInstruction {
                        opcode: Opcode::Add,
                    },
        ...
        assert!(matches!(
            specialized.blocks[0].instructions[0].opcode,
            SpecializedOpcode::LoadLocalFast(0)
        ));
        assert!(matches!(
            specialized.blocks[0].instructions[1].opcode,
            SpecializedOpcode::LoadLocalFast(1)
        ));
        assert!(matches!(
            specialized.blocks[0].instructions[2].opcode,
            SpecializedOpcode::IntAdd
        ));
    }
