...
mod tests {
    ...
    fn test_borrow_checker_basic() {
        ...
        assert!(checker
            .add_borrow("x".to_string(), BorrowType::Immutable, span.clone())
            .is_ok());
        assert!(checker
            .add_borrow("x".to_string(), BorrowType::Immutable, span.clone())
            .is_ok());
        ...
        assert!(checker
            .add_borrow("x".to_string(), BorrowType::Mutable, span.clone())
            .is_err());
    }
    ...
    fn test_borrow_checker_exclusive() {
        ...
        // Mutable borrow OK
        assert!(checker
            .add_borrow("y".to_string(), BorrowType::Mutable, span.clone())
            .is_ok());

        // Any other borrow fails
        assert!(checker
            .add_borrow("y".to_string(), BorrowType::Immutable, span.clone())
            .is_err());
        assert!(checker
            .add_borrow("y".to_string(), BorrowType::Mutable, span.clone())
            .is_err());
    ...
    fn test_borrow_checker_ast() {
        ...
        let ast = AstNode::Block(vec![
            ...
            AstNode::Block(vec![AstNode::LetStmt {
                name: "y".to_string(),
                mutable: false,
                ty: None,
        ...
        assert!(
            checker.check(&ast).is_ok(),
            "Borrow checker failed on valid code: {:?}",
            checker.check(&ast).err()
        );
    }
    ...
    fn test_borrow_checker_fail() {
        ...
        assert!(
            checker.check(&ast).is_err(),
            "Borrow checker should have failed on overlapping borrows"
        );
    }
}
