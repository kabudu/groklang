...
mod optimization_benchmarks {
    use grok::ir::{IRBlock, IRFunction, IRInstruction, Opcode};
    use grok::optimizations::{BytecodeOptimizer, HotPathTracker, OptimizedVM, HOT_THRESHOLD};
    use grok::vm::Value;
            ...
            IRFunction {
                ...
                blocks: vec![
                    IRBlock {
                        ...
                        instructions: vec![
                            IRInstruction {
                                opcode: Opcode::LoadVar("n".to_string()),
                            },
                            IRInstruction {
                                opcode: Opcode::PushInt(2),
                            },
                            IRInstruction { opcode: Opcode::Lt },
                            IRInstruction {
                                opcode: Opcode::JmpIfFalse("else_0".to_string()),
                            },
                        ],
                    },
                    IRBlock {
                        ...
                        instructions: vec![
                            ...
                            IRInstruction {
                                opcode: Opcode::Jmp("end_0".to_string()),
                            },
                        ],
                    },
                    IRBlock {
                        ...
                        instructions: vec![
                            ...
                            IRInstruction {
                                opcode: Opcode::PushInt(1),
                            },
                            IRInstruction {
                                opcode: Opcode::Sub,
                            },
                            IRInstruction {
                                opcode: Opcode::Call("fib".to_string(), 1),
                            },
                            // fib(n - 2)
                            IRInstruction {
                                opcode: Opcode::LoadVar("n".to_string()),
    ...
    fn test_bytecode_specialization_benefit() {
        ...
        let ir_func = IRFunction {
            ...
            blocks: vec![IRBlock {
                ...
                instructions: vec![
                    IRInstruction {
                        opcode: Opcode::LoadVar("x".to_string()),
                    },
                    IRInstruction {
                        opcode: Opcode::LoadVar("y".to_string()),
                    },
                    IRInstruction {
                        opcode: Opcode::Add,
                    },
        ...
        assert!(
            specialized_count >= 6,
            "Expected at least 6 specialized opcodes"
        );
    }
    ...
    fn test_hot_path_triggers_optimization() {
        ...
        assert_eq!(
            became_hot_at, HOT_THRESHOLD,
            "Function should become hot at threshold"
        );
    ...
    fn test_tail_call_optimization_stack_usage() {
        ...
        let block1 = IRBlock {
            ...
            instructions: vec![
                ...
                IRInstruction {
                    opcode: Opcode::Call("recurse".to_string(), 1),
                },
                IRInstruction {
                    opcode: Opcode::Ret,
                },
        ...
        let block2 = IRBlock {
            ...
            instructions: vec![
                ...
                IRInstruction {
                    opcode: Opcode::Call("helper".to_string(), 1),
                },
                IRInstruction {
                    opcode: Opcode::PushInt(1),
                },
        ...
        assert!(
            !is_tail_call(&block2, 1),
            "Should NOT detect as tail call when followed by operations"
        );
    ...
    fn test_specialized_integer_operations() {
        ...
        ctx.execute_specialized(&SpecializedOpcode::LoadLocalFast(0))
            .unwrap();
        assert_eq!(ctx.pop(), Some(Value::Int(100)));

        // Test StoreLocalFast
        ctx.push(Value::Int(200));
        ctx.execute_specialized(&SpecializedOpcode::StoreLocalFast(1))
            .unwrap();
