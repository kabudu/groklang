...
mod optimized_benchmarks {
    ...
    fn benchmark_optimized_fib_30() {
        ...
        assert!(result.is_ok(), "Optimized VM execution failed: {:?}", result.err());
        let val = result.unwrap();
        
        if let Value::Int(v) = val {
            println!("Optimized GrokLang VM fib(30) = {}", v);
            println!("Optimized GrokLang VM Time: {:.4}s", duration.as_secs_f64());
            assert_eq!(v, 832040);
        } else {
